...
int FFT_SC(Complex *input, Complex *output, int logN, int direction);
int REALFFT_SC(int *input, Complex *output, int logN);
...
/* time required: t ~ 52.6 * N * log2(N) / (CPU frequency)                    */ 
/*  (time in microseconds, CPU frequency in MHz)                              */
/* stack used: 68 bytes                                                       */
/* code size ~ 1.7k + N/2 bytes                                               */
                   ...
                   fir_fft_state *s)
{
  ...
  for(i = ndata; i < n; i++) hlp[i] = 0;         /* add zero padding          */
                                                 /* perform forward FFT       */ 
  while(REALFFT_SC(hlp, acc, s->l-1)) ;          /* an error ocurred          */
  ...
  while(FFT(acc, acc, s->l, INVERSE)) ;          /* an error ocurred          */
